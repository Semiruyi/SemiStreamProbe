...
2. RTP Payload Type 与配置不一致；
3. RTP 序列号间断；
4. RTP 重复包；
5. RTP 乱序包；
6. FU-A continuation/end 缺少起始片；
7. FU-A 中间分片丢失；
8. 前一个 FU-A 未完成便出现新的起始片；
9. FU-A 内 SSRC、timestamp、Payload Type 或 NAL Header 变化；
10. STAP-A 长度、空 NAL 或嵌套负载非法；
11. Slice 引用不存在或尚未激活的参数集；
12. IDR NAL 重组失败，以及等待后续完整 IDR 的恢复边界。
